...
def optimal_binning_boundary(x, y, bins = 5):
    ...
    clf = DecisionTreeClassifier(  # “信息熵”最小化准则划分
                                 max_leaf_nodes = bins,  # 最大叶子节点数
                                 min_samples_leaf = 0.05)  # 叶子节点样本数量最小占比  criterion = 'gini',
    ...
    return list(map(lambda x: round(x, 2), boundary))

# ks分箱
def best_ks_box(data, var_name, box_num = 5):
...
# 卡方分箱
def get_chimerge_cutoff(ser, tag, max_groups = 5, threshold=None):
